...
const BenefitCard: React.FC<BenefitCardProps> = ({ benefit, onDetail }) => {
    return (
        <div className={`p-4 mb-3 rounded-xl border ${benefit.isEligible ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800' : 'bg-white dark:bg-bordeaux-900/40 border-slate-200 dark:border-gold-500/15'} shadow-sm transition hover:shadow-md`}>
            <div className="flex justify-between items-start">
                ...
                <div>
                    {benefit.isEligible ? (
                        ...
                        <div className="flex items-center gap-1 px-2 py-1 bg-slate-100 dark:bg-bordeaux-900/60 text-slate-500 dark:text-slate-400 text-xs font-bold rounded-full">
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
            <div className="bg-white dark:bg-bordeaux-950/60 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col border border-slate-200 dark:border-gold-500/20 animate-in fade-in zoom-in duration-200">
                <div className="p-4 border-b border-slate-200 dark:border-gold-500/20 flex justify-between items-center bg-slate-50 dark:bg-bordeaux-950">
                ...
                <div className="p-6 overflow-y-auto">
                    <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="p-3 bg-slate-50 dark:bg-bordeaux-900/40 rounded-lg border border-slate-200 dark:border-gold-500/15">
                    ...
                    <div className="mb-6">
                        ...
                        <p className="text-sm text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-bordeaux-900/40 p-3 rounded-lg border border-slate-200 dark:border-gold-500/15">
                    ...
                    <div>
                        ...
                        <div className="border border-slate-200 dark:border-gold-500/15 rounded-lg overflow-hidden">
                            <table className="w-full text-sm text-left">
                                <thead className="bg-slate-50 dark:bg-bordeaux-900/40 text-xs uppercase font-bold text-slate-500">
                                    <tr>
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-bordeaux-950/60 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-200 dark:border-gold-500/20 animate-in fade-in zoom-in duration-200">
                ...
                {/* Content */}
                <div className="flex-1 overflow-y-auto p-5 bg-slate-50/50 dark:bg-bordeaux-950/60/50 min-h-0">
                    
                    {/* Summary Cards */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                        <div className="bg-white dark:bg-bordeaux-900/40 p-3 rounded-xl border border-slate-200 dark:border-gold-500/15 shadow-sm">
                    ...
                    <div className="flex gap-1 mb-4 border-b border-slate-200 dark:border-gold-500/15 overflow-x-auto">
                        <button 
                            ...
                            className={`px-4 py-2 text-xs font-bold rounded-t-lg transition whitespace-nowrap ${selectedCategory === 'aposentadorias' ? 'bg-white dark:bg-bordeaux-900/40 text-indigo-600 border-x border-t border-slate-200 dark:border-gold-500/15 relative -mb-px' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-bordeaux-900/50/50'}`}
                        >
                            Aposentadorias
                        </button>
                        <button 
                            onClick={() => setSelectedCategory('auxilios')}
                            className={`px-4 py-2 text-xs font-bold rounded-t-lg transition whitespace-nowrap ${selectedCategory === 'auxilios' ? 'bg-white dark:bg-bordeaux-900/40 text-indigo-600 border-x border-t border-slate-200 dark:border-gold-500/15 relative -mb-px' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-bordeaux-900/50/50'}`}
                        >
                            Auxílios e Salários
                        </button>
                        <button 
                            onClick={() => setSelectedCategory('dependentes')}
                            className={`px-4 py-2 text-xs font-bold rounded-t-lg transition whitespace-nowrap ${selectedCategory === 'dependentes' ? 'bg-white dark:bg-bordeaux-900/40 text-indigo-600 border-x border-t border-slate-200 dark:border-gold-500/15 relative -mb-px' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-bordeaux-900/50/50'}`}
                        >
