...
const BenefitCard: React.FC<BenefitCardProps> = ({ benefit, onDetail }) => {
    return (
        <div className={`p-4 mb-3 rounded-xl border ${benefit.isEligible ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800' : 'bg-white dark:bg-bordeaux-950/40 border-slate-200 dark:border-gold-500/15'} shadow-sm transition hover:shadow-md`}>
...
const RMIDetailView = ({ details, onClose }: { details: NonNullable<BenefitResult['rmiDetails']>, onClose: () => void }) => {
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
            <div className="bg-white dark:bg-bordeaux-950/60 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col border border-slate-200 dark:border-gold-500/20 animate-in fade-in zoom-in duration-200">
                <div className="p-4 border-b border-slate-200 dark:border-gold-500/20 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                    <h3 className="font-bold text-lg text-slate-800 dark:text-white">Memória de Cálculo RMI</h3>
                    <button onClick={onClose} className="p-1 hover:bg-slate-200 dark:hover:bg-bordeaux-900/50 rounded-full transition">
                        <XCircleIcon className="h-6 w-6 text-slate-400" />
                    </button>
                </div>
                <div className="p-6 overflow-y-auto">
                    <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="p-3 bg-cream-50 dark:bg-bordeaux-900/40 rounded-lg border border-slate-200 dark:border-gold-500/15">
                    ...
                    <div className="mb-6">
                        ...
                        <p className="text-sm text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-gold-500/15">
                    ...
                    <div>
                        ...
                        <div className="border border-slate-200 dark:border-gold-500/15 rounded-lg overflow-hidden">
                            <table className="w-full text-sm text-left">
                                <thead className="bg-cream-50 dark:bg-bordeaux-900/40 text-xs uppercase font-bold text-slate-500">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {details.salaries.map((s, i) => (
                                        <tr key={i} className="hover:bg-slate-50 dark:hover:bg-bordeaux-900/50/50">
...
const BenefitAnalysisModal: React.FC<BenefitAnalysisModalProps> = ({ isOpen, onClose, data, inpcIndices, ibgeTable }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-bordeaux-950/60 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-200 dark:border-gold-500/20 animate-in fade-in zoom-in duration-200">
                
                {/* Header */}
                <div className="p-5 border-b border-slate-200 dark:border-gold-500/20 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                    <div>
                ...
                <div className="flex-1 overflow-y-auto p-5 bg-slate-50/50 dark:bg-slate-900/50 min-h-0">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                        <div className="bg-white dark:bg-bordeaux-950/40 p-3 rounded-xl border border-slate-200 dark:border-gold-500/15 shadow-sm">
                    ...
                    <div className="flex gap-1 mb-4 border-b border-slate-200 dark:border-gold-500/15 overflow-x-auto">
                        <button 
                            ...
                            className={`px-4 py-2 text-xs font-bold rounded-t-lg transition whitespace-nowrap ${selectedCategory === 'aposentadorias' ? 'bg-white dark:bg-bordeaux-950/40 text-primary-700 border-x border-t border-slate-200 dark:border-gold-500/15 relative -mb-px' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-bordeaux-900/50/50'}`}
                        >
                            Aposentadorias
                        </button>
                        <button 
                            onClick={() => setSelectedCategory('auxilios')}
                            className={`px-4 py-2 text-xs font-bold rounded-t-lg transition whitespace-nowrap ${selectedCategory === 'auxilios' ? 'bg-white dark:bg-bordeaux-950/40 text-primary-700 border-x border-t border-slate-200 dark:border-gold-500/15 relative -mb-px' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-bordeaux-900/50/50'}`}
                        >
                            Auxílios e Salários
                        </button>
                        <button 
                            onClick={() => setSelectedCategory('dependentes')}
                            className={`px-4 py-2 text-xs font-bold rounded-t-lg transition whitespace-nowrap ${selectedCategory === 'dependentes' ? 'bg-white dark:bg-bordeaux-950/40 text-primary-700 border-x border-t border-slate-200 dark:border-gold-500/15 relative -mb-px' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-bordeaux-900/50/50'}`}
                        >
                    ...
                    <div className="space-y-2">
                        {filteredBenefits.length > 0 ? (
                            ...
                            <div className="text-center py-12 text-slate-500 italic border-2 border-dashed border-slate-200 dark:border-gold-500/15 rounded-xl bg-cream-50 dark:bg-bordeaux-900/40/50">
                ...
                {/* Footer */}
                <div className="p-4 border-t border-slate-200 dark:border-gold-500/20 bg-slate-50 dark:bg-slate-950 flex justify-end">
                    <button onClick={onClose} className="px-5 py-2 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm font-bold rounded-lg hover:bg-slate-300 dark:hover:bg-bordeaux-900/60 transition">
                        Fechar
                    </button>
                </div>
            </div>
