        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input 
                ...
                className="w-full p-2 border rounded-lg dark:bg-bordeaux-900/40 dark:border-gold-500/15 mb-2" 
            />
            <select value={clientId} onChange={(e) => setClientId(e.target.value)} className="w-full p-2 border rounded-lg dark:bg-bordeaux-900/40 dark:border-gold-500/15" required>
          ...
          <div>
            ...
            <input type="text" value={referrerName} onChange={(e) => setReferrerName(e.target.value)} className="w-full p-2 border rounded-lg dark:bg-bordeaux-900/40 dark:border-gold-500/15" required />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Total Honorários (R$)</label>
              <input type="number" value={totalFee} onChange={(e) => setTotalFee(Number(e.target.value))} className="w-full p-2 border rounded-lg dark:bg-bordeaux-900/40 dark:border-gold-500/15" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Porcentagem (%)</label>
              <input type="number" value={referrerPercentage} onChange={(e) => setReferrerPercentage(Number(e.target.value))} className="w-full p-2 border rounded-lg dark:bg-bordeaux-900/40 dark:border-gold-500/15" required />
            </div>
          </div>
