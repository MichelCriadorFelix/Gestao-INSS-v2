...
const Login: React.FC<LoginProps> = ({ onLogin, onOpenSettings, isCloudConfigured }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-bordeaux-950 via-bordeaux-900 to-bordeaux-950 px-4 relative overflow-hidden">
      {/* Padrão de balança decorativo */}
      <div className="absolute inset-0 opacity-[0.05] pointer-events-none" style={{backgroundImage: 'radial-gradient(circle at 20% 30%, #C9A961 0%, transparent 30%), radial-gradient(circle at 80% 70%, #C9A961 0%, transparent 30%)'}}></div>
      ...
      <div className="max-w-md w-full bg-bordeaux-950/40 backdrop-blur-xl rounded-2xl shadow-2xl shadow-black/60 p-8 border border-gold-500/20 relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-4xl font-serif font-semibold text-cream-50 tracking-tight">Felix &amp; Castro</h2>
          <div className="flex items-center justify-center gap-2 mt-1">
            <span className="h-px w-8 bg-gold-500/50"></span>
            <p className="text-gold-300/90 font-serif text-sm tracking-[0.2em] uppercase">Advocacia Especializada</p>
            <span className="h-px w-8 bg-gold-500/50"></span>
          </div>
          {!isCloudConfigured && (
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-4">
            <div>
                ...
                type="email"
                required
                className="w-full px-5 py-3 bg-bordeaux-950/60 border border-gold-500/20 rounded-xl focus:ring-2 focus:ring-gold-500 focus:border-transparent outline-none transition text-cream-50 placeholder-cream-100/30"
