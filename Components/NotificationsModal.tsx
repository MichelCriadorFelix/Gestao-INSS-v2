...
const NotificationsModal = ({ isOpen, onClose, notifications, onResolve }: { isOpen: boolean, onClose: () => void, notifications: NotificationItem[], onResolve: (id: string) => void }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-start justify-end z-[90] p-4">
            <div className="bg-white dark:bg-bordeaux-950/60 rounded-2xl shadow-2xl w-full max-w-sm border border-slate-200 dark:border-gold-500/20 mt-16 mr-0 md:mr-4 overflow-hidden animate-in slide-in-from-right duration-200">
                <div className="p-4 border-b border-slate-100 dark:border-gold-500/20 flex justify-between items-center bg-cream-50 dark:bg-bordeaux-900/40/50">
                    <div className="flex items-center gap-2">
                         <div className="bg-orange-100 dark:bg-orange-900/30 p-1.5 rounded-lg text-orange-600 dark:text-orange-400">
                             <BellAlertIcon className="h-5 w-5" />
                         </div>
                         <h3 className="font-serif font-semibold text-slate-800 dark:text-cream-50">Alertas Urgentes</h3>
                    </div>
                    ...
                    {notifications.length === 0 ? (
                        ...
                        <ul className="space-y-1">
                            {notifications.map((notif, idx) => (
                                <li key={`${notif.id}-${idx}`} className="p-3 hover:bg-slate-50 dark:hover:bg-bordeaux-900/50/50 rounded-xl transition border border-transparent hover:border-slate-100 dark:hover:border-slate-700/50 group relative">
                ...
                <div className="p-3 bg-cream-50 dark:bg-bordeaux-900/40/50 border-t border-slate-100 dark:border-gold-500/20 text-center">
