...
const ResolutionNoteModal: React.FC<ResolutionNoteModalProps> = ({ isOpen, onClose, onConfirm, event, user }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[120] p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-bordeaux-950/60 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-gold-500/20 flex flex-col overflow-hidden">
        <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-gold-500/20">
          <div className="flex items-center gap-2">
            <CheckCircleIcon className="h-6 w-6 text-emerald-500" />
            <h3 className="font-bold text-lg text-slate-800 dark:text-white">Concluir Compromisso</h3>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-bordeaux-900/50 transition-colors">
            <XMarkIcon className="h-6 w-6" />
          </button>
        </div>
        
        <div className="p-6 space-y-4">
          <div className="bg-cream-50 dark:bg-bordeaux-900/40/50 p-4 rounded-xl border border-slate-100 dark:border-gold-500/15/50">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full h-32 p-3 rounded-xl border border-slate-200 dark:border-gold-500/15 bg-white dark:bg-bordeaux-950/40 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all resize-none text-sm"
            />
          </div>
        ...
        <div className="p-5 bg-cream-50 dark:bg-bordeaux-900/40/50 border-t border-slate-100 dark:border-gold-500/20 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-gold-500/15 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-white dark:hover:bg-bordeaux-900/50 transition-all"
          >
            Cancelar
          </button>
          <button
